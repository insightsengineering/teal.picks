...
describe("variables() fixed attribute", {
...
describe("values() assertions", {
  ...
  it("values(choices) accepts predicate functions, character, numeric(2), date(2) and posixct(2). No tidyselect", {
    ...
    expect_no_error(values(choices = 1))
    expect_no_error(values(choices = c(1, 2, 3)))
    expect_error(values(choices = c(-Inf, Inf)))
    expect_no_error(values(choices = as.Date(1:2)))
    expect_no_error(values(choices = as.Date(1)))
    expect_no_error(values(choices = as.Date(1:3)))
    expect_no_error(values(choices = as.POSIXct(1:2)))
    expect_no_error(values(choices = as.POSIXct(1)))
    expect_no_error(values(choices = as.POSIXct(1:3)))
...
describe("values() attributes", {
  ...
  it("fixed=TRUE when single choices is provided and selected is same", {
    expect_true(attr(values(choices = "test", selected = "test"), "fixed"))
  })
  ...
  it("fixed=FALSE when single choices is provided and selected is NULL", {
    expect_false(attr(values(choices = "test", selected = NULL), "fixed"))
  })
})
