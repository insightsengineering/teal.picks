...
| Approach | Works with |
|---|---|
| Default behavior | `variables()`, `values()`, and `datasets()` (`selected` only) |
...
For `datasets()`, there is no default for `choices`; define at least one dataset unless `check_no_dataset = FALSE`.
